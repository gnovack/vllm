...
| `flashinfer_cutedsl` | nvfp4: CUDA; **mxfp4/W4A8: SM90 only** | nvfp4; **mxfp4 (W4A8)** | `flashinfer` (CuteDSL) for nvfp4; mxfp4/W4A8 uses the vendored kernel (no flashinfer build needed) | For an NVFP4 checkpoint: FP4-only, `VLLM_FLASHINFER_MOE_BACKEND="masked_gemm"`. For MXFP4 (default `expert_dtype:"fp4"`) on **Hopper**: selects `FLASHINFER_CUTEDSL_MXFP4_FP8` — MXFP4 weights × FP8-e4m3 activations (the kernel casts bf16→FP8 internally; vLLM treats it as W4A16). Vendored from [FlashInfer PR #3516](https://github.com/flashinfer-ai/flashinfer/pull/3516) under `vllm/model_executor/layers/fused_moe/flashinfer_w4a8_mxfp4/`; runs on the bundled `nvidia-cutlass-dsl` + Triton (JIT-compiled). Not in `auto` — opt in with `--moe-backend flashinfer_cutedsl --expert-dtype fp4`. SILU SwiGLU only (no expert biases / clamped-SwiGLU). |

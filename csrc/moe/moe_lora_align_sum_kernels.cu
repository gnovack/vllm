...
__global__ void moe_lora_align_sum_kernel_large_num_experts(
    ...
    int32_t* __restrict__ sorted_token_ids, int32_t* __restrict__ expert_ids,
    int topk_num, int32_t* total_tokens_post_pad, int32_t* num_tokens_per_lora,
    int32_t* adapter_enabled, int32_t* __restrict__ cumsum,
    int32_t experts_per_warp, int32_t padded_num_experts) {
  int lora_id = blockIdx.x;
  if (adapter_enabled[lora_id] * num_tokens_per_lora[lora_id] == 0) {
    return;
  }
  ...
  for (size_t i = tid; i < numel; i += stride) {
    ...
    if (expert_id >= num_experts ||
        token_lora_mapping[i / topk_num] != lora_id) {
      continue;
    }
  ...
  if (threadIdx.x < num_experts) {
    for (int i = cumsum[(num_experts + 1) * lora_id + threadIdx.x];
         i < cumsum[(num_experts + 1) * lora_id + threadIdx.x + 1];
         i += block_size) {
      expert_ids[index(max_num_m_blocks, lora_id, i / block_size)] =
          threadIdx.x;
    }
...
__global__ void lora_count_and_sort_expert_tokens_kernel(
  ...
  for (size_t i = tid; i < numel; i += stride) {
    ...
    int32_t rank_post_pad = atomicAdd(
        &cumsum_buffer[index(num_experts + 1, lora_id, expert_id)], 1);
  ...
  VLLM_DISPATCH_INTEGRAL_TYPES(
      topk_ids.scalar_type(), "moe_lora_align_sum_kernel", [&] {
        ...
        if (small_batch_expert_mode) {
          ...
          auto kernel =
              moe_lora_align_sum_kernel_large_num_experts<scalar_t, int32_t>;
          ...
          // torch::Tensor cumsum = torch::empty({max_loras * (num_experts +
          // 1)}, options_int);
          ...
          sort_kernel<<<gridDims, block_threads, 0, stream>>>(
              ...
              topk_ids.numel(), num_experts, max_num_tokens_padded);
        }
      });
}
        ...
        for module_name, module in self.modules.items():
            ...
            if module_lora:
                ...
                if isinstance(module, FusedMoEWithLoRA) and torch.is_tensor(module_lora.lora_a):
                    gate_up_lora = self._get_lora_layer_weights(lora_model, module_name + ".base_layer")
                    down_proj_lora = module_lora
                    num_experts = module_lora.lora_a.shape[-1] // module_lora.rank

                    gate_proj_a = gate_up_lora.lora_a.chunk(num_experts, dim=-1)
                    up_proj_a = gate_up_lora.lora_a.chunk(num_experts, dim=-1)
                    
                    gate_proj_b = gate_up_lora.lora_b[..., ::2].chunk(num_experts, dim=0)
                    up_proj_b = gate_up_lora.lora_b[..., 1::2].chunk(num_experts, dim=0)

...
def _build_dp_padding(num_tokens: int, dp_padding: str, rank: int, device):
    # is_padding is consumed by CUDA ops (router topk / modular-kernel mask), so
    # it must live on the rank's device, not CPU.
    result = torch.zeros((num_tokens,), dtype=torch.bool, device=device)
    if not dp_padding:
        return result

    padding_by_rank = dp_padding.split(",")
    for p in padding_by_rank:
        if p.split(":")[0] == str(rank):
            num_pad = int(p.split(":")[1])
            if num_pad > 0:
                result[-num_pad:].fill_(True)
    return result
...
def _iter_token_points(args, dp_rank: int):
    """Yield (global_total_tokens, this_rank_tokens) for each data point.

    Default: even DP split of each --num-tokens value. With --dp-tokens
    "n0,n1,..." (one count per DP rank), run a single point with those explicit
    per-rank counts -- this is how we create a *ragged* per-rank distribution,
    which is what drives the AgRs all_gatherv chunk sizes (and thus the
    AllGather imbalance we want to study).
    """
    if args.dp_tokens:
        counts = [int(x) for x in args.dp_tokens.split(",")]
        yield sum(counts), counts[dp_rank]
    else:
        for total in args.num_tokens:
            yield total, _tokens_for_dp_rank(total, dp_rank, args.data_parallel_size)
...
@torch.no_grad()
def benchmark(args, vllm_config, module, hidden_states, input_ids, is_padding):
    def run_once():
...
def _run_worker(local_rank: int, world_size: int, init_method: str, args) -> None:
    ...
    with set_current_vllm_config(vllm_config):
        ...
        results = []
        for total_tokens, num_tokens in _iter_token_points(args, dp_rank):
            torch.manual_seed(args.seed + dp_rank)
            hidden_states, input_ids = make_inputs(
                num_tokens, vllm_config, dtype, device
            )
            torch.distributed.barrier()
            is_padding = _build_dp_padding(num_tokens, args.dp_padding, rank, device)
            if args.dp_padding:
                print(
                    f"[rank {rank}] tokens={num_tokens} "
                    f"is_padding: device={is_padding.device} "
                    f"num_pad={int(is_padding.sum())} "
                    f"skip_padding_env={__import__('vllm').envs.VLLM_MOE_SKIP_PADDING}",
                    flush=True,
                )
            latencies_ms = benchmark(
                args, vllm_config, module, hidden_states, input_ids, is_padding
            )
...
def main(args):
    ...
    mp.spawn(
        _run_worker,
        args=(world_size, f"tcp://127.0.0.1:{get_open_port()}", args),
        nprocs=world_size,
        join=True,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Per-rank is_padding tail lengths as 'rank:n,rank:n' (needs "
        "VLLM_MOE_SKIP_PADDING=1). Marks the last n rows of that rank's batch as "
        "padding so the router drops them from the experts. Affects expert "
        "compute only -- not the all_gatherv chunk sizes.",
    )
    parser.add_argument(
        "--dp-tokens",
        type=str,
        default=None,
        metavar="n0,n1,...",
        help="Explicit per-DP-rank token counts (one per rank), overriding the "
        "even --num-tokens split. Use to create a ragged per-rank distribution "
        "and study its effect on the AgRs all_gatherv (AllGather) cost.",
    )
    args = parser.parse_args()
    if args.dp_tokens:
        counts = [int(x) for x in args.dp_tokens.split(",")]
        if len(counts) != args.tensor_parallel_size * args.data_parallel_size:
            raise SystemExit(
                f"--dp-tokens needs one count per rank "
                f"(TP*DP={args.tensor_parallel_size * args.data_parallel_size}); "
                f"got {len(counts)}."
            )
    main(args)